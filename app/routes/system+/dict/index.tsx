import { EditOutlined, MinusOutlined, PlusOutlined, SisternodeOutlined } from "@ant-design/icons";
import { useLoaderData, useRevalidator } from "@remix-run/react";
import { App, Button, Input, Tooltip } from "antd";
import { Suspense, useRef, useState } from "react";
import { delDictType, getDictTypeList } from "../api";
import SettingMenuModal, { type SettingDictTypeRefType } from "./components/SettingDictType";
...
export default function Dict() {
  ...
  const [selectedDictType, setSelectedDictType] = useState<DictType>(dictTypeList?.[0]);
    ...
    <>
      <div className="border-b flex items-center pb-6 pt-3 px-4 mb-4">
        <div className="flex-1">
          <h3 className="text-lg font-500 text-primary">字典管理</h3>
          <p className="text-color/60">管理系统字典/新增、编辑字典类型/字典值、删除等功能</p>
        </div>
        <div className="space-x-4">
          <Button type="primary" icon={<SisternodeOutlined />} onClick={() => settingDictType()}>
            新增字典类型
          </Button>
        </div>
      </div>
                ...
                >
                  ...
                  <div
                    className={`space-x-2 hidden ${
                      selectedDictType?.dictType === item.dictType ? "!block" : ""
                    }`}
                  >
        ...
        <div className="flex-1 bg-white dark:bg-[#141414] rounded p-4">
          ...
          <div className="grid grid-cols-3 gap-3 my-5">
            <div>
              ...
              <span className="text-primary font-500"> {selectedDictType?.dictName}</span>
            </div>
            <div>
              <span className="text-color/60">字典类型：</span> {selectedDictType?.dictType}
            </div>
            <div>
              <span className="text-color/60">字典编号：</span> {selectedDictType?.dictId}
            </div>
            <div>
            ...
            <div>
              <span className="text-color/60">备注：</span> {selectedDictType?.remark}
            </div>
            <div>
              <span className="text-color/60">创建时间：</span> {selectedDictType?.createTime}
            </div>
          </div>
          <h3 className="pl-3 mt-6 text-base border-l-4 border-primary">字典标签</h3>
          <Suspense fallback={<Loading className="!pt-20" />}>
            {selectedDictType.dictType && <ShowDictValue dictType={selectedDictType.dictType} />}
          </Suspense>
        </div>
      </div>
