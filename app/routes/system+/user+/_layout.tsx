...
export default function UserList() {
  ...
  const columns: ColumnsType<RecordType> = [
    ...
    { title: '部门', dataIndex: 'dept', ellipsis: true, render: text => text?.deptName },
    ...
    { title: '创建时间', dataIndex: 'createTime', width: 190 },
    {
      ...
      render: (_, record) => (
        <div className="space-x-2">
          <Link to={`/system/user/setting?id=${record.userId}`} state={record} prefetch="intent">
            修改
          </Link>
  ...
  const searchForm = (
    <div className="mb-4 flex-between">
      <Form form={form} layout="inline" variant="filled" onFinish={submit} autoComplete="off">
        ...
        <Form.Item name="status">
          <Select placeholder="状态" className="!w-[200px]" options={sys_normal_disable} />
        </Form.Item>
        <Form.Item noStyle>
